...
class Agent:
    def __init__(self, medical_report=None, role=None, extra_info=None):
        ...
        self.model = ChatGroq(temperature=0, model="llama3-8b-8192") # Or "llama3-70b-8192", "mixtral-8x7b-32768"
    ...
    def create_prompt_template(self):
        if self.role == "MultidisciplinaryTeam":
            ...
            return PromptTemplate.from_template(templates_dict[self.role])
    ...
    def run(self):
        ...
        try:
            if self.role == "MultidisciplinaryTeam":
                # The prompt_template for MultidisciplinaryTeam is already a formatted string
                # encapsulated in a PromptTemplate object.
                # ChatGroq.invoke expects a str, PromptValue, or list of BaseMessages.
                # Convert the PromptTemplate to a string or a HumanMessage.
                
                # Option 1: Convert to string (simpler for single-turn text prompts)
                response = self.model.invoke(self.prompt_template.format()) # .format() without args for already templated string
                
                # Option 2: Wrap in a HumanMessage (more robust for chat models)
                # response = self.model.invoke([HumanMessage(content=self.prompt_template.format())])
            else:
                response = self.model.invoke(self.prompt_template.format(medical_report=self.medical_report))
...
# Define specialized agent classes (no changes needed here as they correctly inherit)
